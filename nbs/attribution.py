...
#%%
def visualize_gradcam(image, tensor, title):
    # Get the Grad-CAM output
    grayscale_cam = cam(input_tensor=tensor)
    
    # Resize the heatmap to match the input image dimensions
    heatmap = cv2.resize(grayscale_cam[0], (image.size[0], image.size[1]))
    
    # Convert PIL image to numpy array and normalize
    img_array = np.array(image) / 255.0
    
    # Create the visualization
    visualization = show_cam_on_image(img_array, heatmap)
...
print(f"\nVisualizing Grad-CAM for Newt {random_newts[0]}:")
visualize_gradcam(newt1_img1, newt1_tensor1, f"Newt {random_newts[0]} - Image 1")
visualize_gradcam(newt1_img2, newt1_tensor2, f"Newt {random_newts[0]} - Image 2")

print(f"\nVisualizing Grad-CAM for Newt {random_newts[1]}:")
visualize_gradcam(newt2_img1, newt2_tensor1, f"Newt {random_newts[1]} - Image 1")
visualize_gradcam(newt2_img2, newt2_tensor2, f"Newt {random_newts[1]} - Image 2")

#%%
# Calculate similarity between images
def calculate_similarity(tensor1, tensor2):
    with torch.no_grad():
        features1 = model(tensor1.to(device))
        features2 = model(tensor2.to(device))
...
print("\nSimilarity Analysis:")
print(f"Same newt similarity: {calculate_similarity(newt1_tensor1, newt1_tensor2):.4f}")
print(f"Different newts similarity: {calculate_similarity(newt1_tensor1, newt2_tensor1):.4f}")
...
#%%
# Set up Grad-CAM for the similarity model
target_layers = [similarity_model.backbone.layers[-1].blocks[-1].norm1]
cam = GradCAM(model=similarity_model, target_layers=target_layers)

#%%
def visualize_similarity_gradcam(image1, tensor1, image2, tensor2, title):
    # Get the Grad-CAM output
    grayscale_cam = cam(input_tensor=[tensor1, tensor2])
    
    # Resize the heatmap to match the input image dimensions
    heatmap = cv2.resize(grayscale_cam[0], (image1.size[0], image1.size[1]))
    ...
    with torch.no_grad():
        similarity = similarity_model(tensor1.to(device), tensor2.to(device))
    ...
    plt.subplot(1, 3, 3)
    plt.imshow(visualization)
    plt.title(f"Grad-CAM (Similarity: {similarity.item():.4f})")
    plt.axis('off')
    plt.show()

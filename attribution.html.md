...
<a
href="https://github.com/mshahoyi/gcn-reid/blob/main/gcn_reid/attribution.py#L9"
...
\*Perform occlusion sensitivity test on the first image to see which
regions affect similarity with the second image.

Args: model: The similarity model image1: First image tensor (to be
occluded) - shape \[1, C, H, W\] image2: Second image tensor - shape
\[1, C, H, W\] patch_size: Size of the occlusion patch stride: Stride
for moving the occlusion patch occlusion_value: Value used for occlusion
(default: 0)

Returns: Sensitivity map showing which regions, when occluded, affect
similarity the most\*
